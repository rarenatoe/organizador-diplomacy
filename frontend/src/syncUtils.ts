...
import type { SimilarName, NotionPlayer, EditPlayerRow, OrganizarValidation } from "./types";
 ...
 */
export function validateOrganizar(players: EditPlayerRow[]): OrganizarValidation | null {
  ...
  if (isAllOnes || excludedPlayers.length > 0 || hasGmShortage) {
    return {
      isAllOnes,
      gmShortage: hasGmShortage ? { required: projectedTables, assigned: assignedGMs } : null,
      excludedPlayers,
    };
  }
