...
async def make_snapshot_with_players(db_session: Any, count: int = 14) -> int:
    ...
    for i in range(count):
        ...
        await add_player_to_snapshot(
            db_session, snap_id, pid, "Antiguo", i % 3, 0, 2, 0
        )
...
class TestApiGameDraft:
    ...
    async def test_valid_snapshot_returns_draft(
    ...
    async def test_draft_does_not_write_to_db(
        self, client: Any, db_session: Any
    ) -> None:
        ...
        result = await db_session.execute(
            select(Event).where(Event.type == "game")
        )
        event_count = len(result.scalars().all())
        assert event_count == 0

    async def test_too_few_players_returns_400(
        self, client: Any, db_session: Any
    ) -> None:
...
class TestApiGameSave:
    ...
    async def test_missing_snapshot_id_returns_400(
        ...
        draft_resp = await client.post(
            "/api/game/draft", json={"snapshot_id": snap_id}
        )
        draft = draft_resp.json()
        resp = await client.post("/api/game/save", json={"draft": draft})
        assert resp.status_code == 422  # FastAPI validation error

    async def test_save_draft_returns_game_id(
        self, client: Any, db_session: Any
    ) -> None:
        snap_id = await make_snapshot_with_players(db_session, 14)
        draft_resp = await client.post(
    ...
    async def test_save_draft_creates_game_event_in_db(
        self, client: Any, db_session: Any
    ) -> None:
        snap_id = await make_snapshot_with_players(db_session, 14)
        draft_resp = await client.post(
        ...
        result = await db_session.execute(
            select(Event).where(Event.id == game_id)
        )
    ...
    async def test_save_draft_persists_correct_mesa_count(
        ...
        game_id = save_resp.json()["game_id"]
        result = await db_session.execute(
            select(Mesa).where(Mesa.event_id == game_id)
        )
        mesas = result.scalars().all()
        assert len(mesas) == len(draft["mesas"])

    async def test_save_draft_creates_output_snapshot(
        self, client: Any, db_session: Any
    ) -> None:
